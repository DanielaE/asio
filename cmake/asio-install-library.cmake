...
#
# name
#   Logical package name (e.g. "asio_any").
...
# ----------------
#
# ASIO_INSTALL_CONFIG_FILE_PACKAGES
...
function(asio_install_library name)
    # ----------------------------
    # Argument parsing
    # ----------------------------
    ...
    if(CMAKE_SKIP_INSTALL_RULES)
        message(
            WARNING
            "asio_install_library(${name}): not installing targets '${ASIO_TARGETS}' due to CMAKE_SKIP_INSTALL_RULES"
        )
        return()
    endif()
    ...
    if(NOT ASIO_NO_VERSION_SUFFIX)
        set(_version_suffix "-${PROJECT_VERSION}")
        set(_include_install_dir DESTINATION ${CMAKE_INSTALL_INCLUDEDIR}/asio${_version_suffix})
        # set(_lib_install_dir DESTINATION ${CMAKE_INSTALL_LIBDIR}/asio${_version_suffix})
        # set(_bin_install_dir DESTINATION ${CMAKE_INSTALL_BINDIR}/asio${_version_suffix})
    endif()
    ...
    # Defaults
    # ----------------------------
    if(NOT ASIO_NAMESPACE)
        set(ASIO_NAMESPACE "asio::")
    endif()

    if(NOT ASIO_EXPORT_NAME)
        set(ASIO_EXPORT_NAME "${name}-targets")
    endif()

    if(NOT ASIO_DESTINATION)
        set(ASIO_DESTINATION "${_config_install_dir}/modules")
    endif()
    ...
    foreach(_tgt IN LISTS ASIO_TARGETS)
        ...
        # # Given asio_any, the component_name is any
        # string(REPLACE "_" ";" name_parts "${_tgt}")
        # # fail if the name doesn't look like asio_any
        # list(LENGTH name_parts name_parts_length)
        # if(NOT name_parts_length GREATER_EQUAL 2)
        #     message(
        #         FATAL_ERROR
        #         "asio_install_library(${name}): expects a name of the form 'asio_<name>', got '${_tgt}'"
        #     )
        # endif()
        ...
        if(_available_header_sets)
            message(
                VERBOSE
                "asio-install-library(${name}): '${_tgt}' has INTERFACE_HEADER_SETS=${_available_header_sets}"
            )
    ...
    elseif(ASIO_INSTALL_CONFIG_FILE_PACKAGES)
        list(FIND ASIO_INSTALL_CONFIG_FILE_PACKAGES "${name}" _idx)
        if(NOT _idx EQUAL -1)
            set(_install_config ON)
        endif()
    endif()
    ...
    if(_install_config)
        ...
        message(
            WARNING
            "asio-install-library(${name}): Not installing a config package for '${name}'"
        )
    endif()
endfunction()
